import React from 'react'
import { download, download2, clipboard } from '../assets'
import { downloadImage, copyPrompt } from '../utils'

type PhotoInfo = {
  _id: number;
  name: string;
  prompt: string;
  photo: string;
};

const Card = ({ _id, name, prompt, photo }: PhotoInfo): JSX.Element => {
  return (
    <div className='rounded-xl group relative shadow-card hover:shadow-cardhover card'>
      <img
        src={photo}
        alt={prompt}
        className='w-full h-auto object-cover rounded-xl'
      />
      <div className='group-hover:flex flex-col max-h-[94.5%] hidden absolute bottom-0 left-0 right-0 bg-[#10131f] m-2 p-4 rounded-md'>
        <p className='text-white text-sm overflow-y-auto prompt'>
          {prompt.charAt(0).toUpperCase() + prompt.slice(1)}
        </p>

        <div className='mt-5 flex justify-between gap-2'>
          <div className='flex items-center gap-2'>
            <div className='w-7 h-7 rounded-full object-cover bg-purple-700 flex justify-center items-center text-white text-xs font-bold'>
              {name[0]}
            </div>

            <p className='text-white text-sm'>
              {name}
            </p>
          </div>

          <div className='flex items-right'>
            <button
              type='button'
              onClick={() => copyPrompt(_id, prompt)}
              className='outline-none bg-transparent border-none mx-2'
            >
              <img
                src={clipboard}
                alt='clipboard'
                className='w-6 h-6 object-contain invert'
              />
            </button>

            <button
              type='button'
              onClick={() => downloadImage(_id, photo)}
              className='outline-none bg-transparent border-none mx-2'
            >
              <img
                src={download2}
                alt='download'
                className='w-6 h-6 object-contain invert'
              />
            </button>
          </div>
        </div>
      </div>
    </div>
  )
}

export default Card